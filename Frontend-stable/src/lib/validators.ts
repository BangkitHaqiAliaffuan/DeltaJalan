...
export function validateIndonesianPhone(input: string): PhoneValidationResult {
  ...
  if (!PHONE_REGEX.test(cleaned)) {
    return {
      valid: false,
      normalized: cleaned,
      error: "Format nomor telepon tidak valid. Gunakan format: 08xxxxxxxxxx atau +628xxxxxxxxxx.",
    };
  }
...
export function validateNamaLengkap(input: string): NameValidationResult {
  ...
  if (!NAME_REGEX.test(trimmed)) {
    return {
      valid: false,
      normalized: trimmed,
      error: "Nama lengkap hanya boleh mengandung huruf, spasi, titik, dan tanda hubung.",
    };
  }
  ...
  if (/^[ '.-]/.test(trimmed) || /[ '.-]$/.test(trimmed)) {
    return {
      ...
      normalized: trimmed,
      error:
        "Nama lengkap tidak boleh diawali atau diakhiri dengan spasi, titik, atau tanda hubung.",
    };
  }
  ...
  if (!/[A-Za-zÀ-ÖØ-öø-ÿ]/.test(trimmed)) {
    return {
      valid: false,
      normalized: trimmed,
      error: "Nama lengkap harus mengandung minimal satu huruf.",
    };
  }
