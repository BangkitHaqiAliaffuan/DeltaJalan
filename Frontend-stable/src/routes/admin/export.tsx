...
function AdminExport() {
  ...
  const statsQuery = useQuery({
    ...
    queryFn: () => apiFetch(`/api/reports/stats`, { headers: { Authorization: `Bearer ${token}` } }).then((r) => r.json()),
  });

  const stats = statsQuery.data as {
    data?: { total: number; selesai: number; sedang_diperbaiki: number; rusak_berat: number; rusak_sedang: number; rusak_ringan: number };
  } | undefined;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-5">
          ...
          <div className="space-y-2">
            <button onClick={() => doDownload("excel")} disabled={dlBusy !== null} className="w-full flex items-center justify-between px-4 py-3 rounded-lg border border-[#E2E8F0] hover:bg-[#F8FAFC] transition-colors text-[13px] disabled:opacity-50">
            ...
            <button onClick={() => doDownload("pdf")} disabled={dlBusy !== null} className="w-full flex items-center justify-between px-4 py-3 rounded-lg border border-[#E2E8F0] hover:bg-[#F8FAFC] transition-colors text-[13px] disabled:opacity-50">
              <span className="flex items-center gap-2">
