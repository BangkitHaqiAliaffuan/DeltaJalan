...
export const Route = createFileRoute("/lacak")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Lacak laporan kerusakan jalan Anda menggunakan kode laporan." },
    ],
  }),
});
    ...
    <div className="relative min-h-[100dvh] w-full overflow-hidden">
      ...
      <div className="relative z-10 min-h-[100dvh] flex items-center justify-center p-4">
        <div className="w-full max-w-[400px] animate-fade-in">
          <div className="bg-white rounded-2xl overflow-hidden border-2 border-[#1e40af]" style={{ boxShadow: "0 25px 60px rgba(0,0,0,0.3)" }}>
              ...
              <div className="px-6 pb-6 border-t border-[#E2E8F0] pt-4">
                <div className="mb-4">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-mono text-sm font-bold text-[#0F172A]">{data.report.report_code}</span>
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${getStatusBadge(data.report.status).color}`}>
                      {getStatusBadge(data.report.status).label}
                    </span>
                  </div>
                  <p className="font-label-md text-label-md font-semibold text-[#0F172A]">{data.report.road_name}</p>
                        ...
                        return (
                          <div key={i} className="flex gap-3 pb-3 relative">
                            <div className="flex flex-col items-center">
                              <div className={`w-2.5 h-2.5 rounded-full ${isLast ? "bg-[#1e40af]" : "bg-[#c4c5d5]"} shrink-0 mt-1`} />
