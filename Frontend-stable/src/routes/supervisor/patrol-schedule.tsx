...
const ALL_HARI: Hari[] = ["Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu"];
...
function PatrolSchedulePage() {
  ...
  async function handleSubmit() {
    setFormError("");
    if (!form.team_id) {
      setFormError("Pilih tim satgas.");
      return;
    }
    if (form.hari.length < 2) {
      setFormError("Pilih minimal 2 hari dalam seminggu.");
      return;
    }
    if (form.kecamatan_list.length === 0) {
      setFormError("Pilih minimal 1 kecamatan.");
      return;
    }
      ...
      <section className="bg-gradient-to-br from-[#1e40af] to-[#2e68d8] p-5 text-white mb-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-blue-200 mt-1">Atur pola patroli rutin tim satgas</p>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        {schedulesQuery.isPending ? (
          ...
          <div className="text-center py-16 text-[#476788]">
            ...
            <p className="text-[13px] text-[#64748B] mb-4">
              Buat jadwal pertama untuk mengatur pola patroli tim
            </p>
            <button
              onClick={openCreate}
        ...
        {modal === "create" || modal === "edit" ? (
          ...
          >
            ...
            <div>
              ...
              <SearchSelect
                ...
                onChange={(v) => {
                  ...
                  setForm({ ...form, team_id: v as string, kecamatan_list: kec });
                }}
            ...
            <div>
              <label className="block text-[13px] font-semibold text-[#0F172A] mb-2">
                Hari Patroli <span className="text-[#E11D48]">*</span>
              </label>
              ...
              <p
                className={`text-[11px] mt-1.5 ${form.hari.length > 0 && form.hari.length < 2 ? "text-[#E11D48]" : "text-[#64748B]"}`}
              >
                Terpilih: {form.hari.length ? form.hari.join(", ") : "—"}
                {form.hari.length > 0 && form.hari.length < 2 && " (minimal 2)"}
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 p-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg min-h-[40px]">
                {form.kecamatan_list.length > 0 ? (
                  form.kecamatan_list.map((k) => (
                    <span
                      key={k}
                      className="px-2.5 py-1 bg-[#EEF3FA] rounded-md text-[12px] font-medium text-[#1e40af]"
                    >
                      {k}
                    </span>
                  ))
              ...
              {form.kecamatan_list.length > 0 && (
                <p className="text-[11px] text-[#94A3B8] mt-1.5">
                  {form.kecamatan_list.length} kecamatan — otomatis berdasarkan wilayah tim
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[13px] font-semibold text-[#0F172A] mb-1">
                  Mulai <span className="text-[#E11D48]">*</span>
                </label>
                <input
                  type="date"
        ...
        {modal === "generate" ? (
          ...
          >
            ...
            <p className="text-[12px] text-[#64748B]">
              Generate shift untuk rentang tanggal di atas berdasarkan pola jadwal.
            </p>
          </ModalBase>
        ) : null}
        ...
        {deleteId !== null && (
          ...
          >
            <p className="text-[13px] text-[#475569] leading-relaxed">
              Yakin ingin menghapus jadwal ini? Shift aktif yang belum memiliki laporan akan ikut
              dihapus.
            </p>
          </ModalBase>
        )}
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-xl p-5 hover:shadow-sm transition-shadow">
      ...
      <div className="flex items-center justify-between text-[11px] text-[#64748B] mb-3">
        ...
        <span
          className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${
            schedule.status === "aktif" ? "bg-green-50 text-[#10B981]" : "bg-gray-50 text-[#94A3B8]"
          }`}
        >
      ...
      <div className="flex items-center gap-1 pt-3 border-t border-[#E2E8F0]">
        <button
          onClick={onEdit}
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold text-[#475569] hover:bg-[#F1F5F9] transition-colors"
        >
          <Icon name="edit" className="!text-[14px]" /> Edit
        </button>
        <button
          onClick={onGenerate}
          className="hidden flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold text-[#1e40af] hover:bg-[#EEF3FA] transition-colors"
        >
          <Icon name="refresh" className="!text-[14px]" /> Generate
        </button>
        <button
          onClick={onDelete}
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold text-[#E11D48] hover:bg-red-50 transition-colors ml-auto"
        >
