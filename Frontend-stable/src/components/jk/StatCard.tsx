    ...
    >
      <div className="flex items-center justify-center gap-1.5">
        ...
        <span className="text-2xl font-bold" style={{ color: iconColor }}>
          {value}
          {suffix && (
            <span className="text-sm font-medium ml-0.5" style={{ color: iconColor, opacity: 0.7 }}>
              {suffix}
            </span>
          )}
        </span>
      </div>
      <p className="text-sm font-medium text-center" style={{ color: textColor, opacity: 0.8 }}>
        {label}
      </p>
    </div>
  );
...
export const SEVERITY_BERAT = {
  from: "#FEF2F2",
  border: "#FECACA",
  icon: "#DC2626",
  text: "#991B1B",
};
export const SEVERITY_SEDANG = {
  from: "#FFF7ED",
  border: "#FED7AA",
  icon: "#EA580C",
  text: "#9A3412",
};
export const SEVERITY_RINGAN = {
  from: "#FFFBEB",
  border: "#FDE68A",
  icon: "#D97706",
  text: "#92400E",
...
export const STATUS_STYLES: Record<
  string,
  { from: string; border: string; icon: string; text: string }
> = {
...
export const TRUST_STYLES: Record<
  string,
  { from: string; border: string; icon: string; text: string; label: string }
> = {
  hijau: {
    from: "#ECFDF5",
    border: "#A7F3D0",
...
export function statusGradient(s: string) {
  return STATUS_STYLES[s]?.from ?? "#FFFBEB";
}
export function statusBorder(s: string) {
  return STATUS_STYLES[s]?.border ?? "#FDE68A";
}
export function statusIconColor(s: string) {
  return STATUS_STYLES[s]?.icon ?? "#D97706";
}
export function statusTextColor(s: string) {
  return STATUS_STYLES[s]?.text ?? "#92400E";
}
export function trustLabel(l: TrustLabel) {
  return TRUST_STYLES[l]?.label ?? "Diragukan";
}
export function trustGradient(l: TrustLabel) {
  return TRUST_STYLES[l]?.from ?? "#FEF2F2";
}
export function trustBorder(l: TrustLabel) {
  return TRUST_STYLES[l]?.border ?? "#FECACA";
}
export function trustIconColor(l: TrustLabel) {
  return TRUST_STYLES[l]?.icon ?? "#DC2626";
}
export function trustTextColor(l: TrustLabel) {
  return TRUST_STYLES[l]?.text ?? "#991B1B";
}
